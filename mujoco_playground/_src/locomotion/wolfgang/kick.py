...
class Kick(wolfgang_base.WolfgangEnv):
  """Kick the Ball."""
  ...
  def _post_init(self) -> None:
    ...
    # Note: First joint is freejoint.  # TODO fix all indices
    ...
    sensor_id = self._mj_model.sensor(f"ball_global_linvel").id
    sensor_adr = self._mj_model.sensor_adr[sensor_id]
    sensor_dim = self._mj_model.sensor_dim[sensor_id]
    self._ball_linvel_sensor_adr = jp.array([list(range(sensor_adr, sensor_adr + sensor_dim))])
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    # Sample the initial pose of the robot.
  ...
  def _get_obs(
    ...
    ball_pos = self._get_uv_ball_pos(data)
    info["rng"], noise_rng = jax.random.split(info["rng"])
    noisy_ball_pos = (
        ball_pos
        + jax.random.normal(noise_rng, shape=ball_pos.shape)
        * self._config.noise_config.level
    ...
    # Ball velocity
    ball_vels = data.sensordata[self._ball_linvel_sensor_adr].ravel()
    ball_vel = jp.linalg.norm(ball_vels)

    privileged_state = jp.hstack([
        # TODO add ball Z and ball velocity
        state,
        ball_vel,  # 1
        ball_pos,  # 2
        gyro,  # 3
  ...
  def _get_uv_ball_pos(self, data: mjx.Data) -> jax.Array:
    """Get the ball position in the local frame of the robot."""
    ball_pos = data.site_xpos[self._ball_site_id]
    torso_pos = data.site_xpos[self._site_id]  # TODO this uses the imu as reference, we want the base foorprint instead
    torso_mat = data.site_xmat[self._site_id]

    # Transform the ball position to the local frame of the robot.
    ball_pos_local = jp.dot(
        torso_mat.T, ball_pos - torso_pos
    )  # Local frame of the robot.

    return ball_pos_local[:2]
  ...
  def _get_reward(
    ...
    return {
        ...
        # Kick related rewards.
        "orient_to_ball": self._orient_to_ball_reward(
        ...
        "ball_travel": self._ball_travel_reward(
            data.site_xpos[self._ball_site_id],
            info["initial_ball_pos"],
        ),
        "ball_speed": self._ball_speed_reward(
            data.sensordata[self._ball_linvel_sensor_adr].ravel()
        ),
  ...
  def _orient_to_ball_reward(
      self,
      ball_uv: jax.Array) -> jax.Array:
    """Reward for orienting towards the ball."""
    # Ball is in the local frame of the robot.
    ball_angle = jp.arctan2(ball_uv[1], ball_uv[0])
    return jp.exp(-jp.abs(ball_angle))

  def _ball_speed_reward(
      self,
  ...
  def _ball_height_reward(
    ...
    """Reward for the ball height."""
    # Ball is in global coordinates.
    return ball_pos[2]

  def _ball_travel_reward(
      self,
      ball_pos: jax.Array,
      initial_ball_pos: jax.Array,
  ) -> jax.Array:
    """Reward for the ball travel distance."""
    # Ball is in global coordinates.
  ...
  def reached_ball(
    ...
    # Compute the angle between the robot and the ball.
    ball_angle = jp.arctan2(ball_uv[1], ball_uv[0])

    # Only reward if the ball is within a certain distance and within a certain angle.
