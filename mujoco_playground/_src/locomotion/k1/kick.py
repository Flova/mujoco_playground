...
from mujoco_playground._src.locomotion.k1 import base as k1_base
from mujoco_playground._src.locomotion.k1 import k1_constants as consts
  ...
  return config_dict.create(
      ...
      reward_config=config_dict.create(
          scales=config_dict.create(
              ...
              kick_dir_bonus=0.0,
              ball_too_fast=0.0,
              orient_to_kick_dir=0.3,
              sidestep=0.3,
              aligned_approach=0.5,
              wrong_approach=-0.1,
              too_fast=0.0,
              symmetry=-0.05,
              lin_vel_z=0.0,
      ...
      ball_distance=[0.32, 0.47],
      too_fast_threshold=2.0,
  )


class Kick(k1_base.K1Env):
  """Walk to ball and kick it — K1 humanoid."""
  ...
  def _post_init(self) -> None:
    ...
    # K1 joint order in qpos[7:19]:
    #   Left_Hip_Pitch(0), Left_Hip_Roll(1), Left_Hip_Yaw(2), Left_Knee_Pitch(3),
    #   Left_Ankle_Pitch(4), Left_Ankle_Roll(5),
    #   Right_Hip_Pitch(6), Right_Hip_Roll(7), Right_Hip_Yaw(8), Right_Knee_Pitch(9),
    #   Right_Ankle_Pitch(10), Right_Ankle_Roll(11)
    self._hip_indices = jp.array([
        self._mj_model.joint("Left_Hip_Pitch").qposadr - 7,
        self._mj_model.joint("Left_Hip_Roll").qposadr - 7,
        self._mj_model.joint("Right_Hip_Pitch").qposadr - 7,
        self._mj_model.joint("Right_Hip_Roll").qposadr - 7,
    ])
    self._knee_indices = jp.array([
        self._mj_model.joint("Left_Knee_Pitch").qposadr - 7,
        self._mj_model.joint("Right_Knee_Pitch").qposadr - 7,
    ])
    # Kick leg: left.
    self._l_hip_pitch_idx = int(
        self._mj_model.joint("Left_Hip_Pitch").qposadr - 7
    )
    self._l_calf_idx = int(self._mj_model.joint("Left_Knee_Pitch").qposadr - 7)

    # Low weight for pitch/knee (swing a lot), high weight for roll/yaw/ankle.
    self._weights = jp.array([
        0.01, 1.0, 1.0, 0.01, 1.0, 1.0,  # L: HP, HR, HY, KP, AP, AR
        0.01, 1.0, 1.0, 0.01, 1.0, 1.0,  # R
    ])
    ...
    # Mirror: swap L/R legs, negate roll/yaw/ankle_roll across sides.
    # K1 L-first: HP(0),HR(1),HY(2),KP(3),AP(4),AR(5), HP(6),HR(7),HY(8),KP(9),AP(10),AR(11)
  ...
  def reset(self, rng: jax.Array) -> mjx_env.State:
    ...
    rng, key = jax.random.split(rng)
    max_ball_speed = jax.random.uniform(key, minval=0.3, maxval=8.0)
  ...
  def step(self, state: mjx_env.State, action: jax.Array) -> mjx_env.State:
    ...
    ball_speed = jp.linalg.norm(data.sensordata[self._ball_linvel_sensor_adr])
    ball_stationary = ball_speed < 0.05
    should_resample = (
  ...
  def _get_obs(
    ...
    _ball_dist = jp.linalg.norm(ball_pos_obs)
    _ball_speed_obs = jp.linalg.norm(data.sensordata[self._ball_linvel_sensor_adr])
    _ball_noise_std = (
        (0.02 + 0.08 * _ball_dist)
        + 0.4 * _ball_speed_obs
    )
    info["rng"], noise_rng = jax.random.split(info["rng"])
    noisy_ball_pos = (
    ...
    true_kick_dir_local = (torso_mat.T @ kick_dir_world_3d)[:2]
    info["rng"], noise_rng = jax.random.split(info["rng"])
    angle_noise = jax.random.normal(noise_rng) * (10.0 * jp.pi / 180.0)
    c, s = jp.cos(angle_noise), jp.sin(angle_noise)
    kick_dir_local = jp.array([[c, -s], [s, c]]) @ true_kick_dir_local
    ...
    state = jp.hstack([
        ...
        phase,                                                             # 4
        kick_dir_local,                                                    # 2
        jp.array([info["max_ball_speed"] / 8.0]),                          # 1
    ])
    ...
    privileged_state = jp.hstack([
        state,
        ball_vel,                                                          # 1
        true_ball_pos_local,                                               # 2
        true_kick_dir_local,                                               # 2
        data.site_xpos[self._ball_site_id][2:3],                           # 1
        true_gyro,                                                         # 3
        accelerometer,                                                     # 3
        true_gravity,                                                      # 3
        linvel,                                                            # 3
  ...
  def _get_reward(
    ...
    return {
        ...
        "move_to_ball": self._reward_move_to_ball(ball_pos_local, linvel),
        "too_fast": (speed > self._config.too_fast_threshold).astype(jp.float32),
        "stop_for_kick": self._reward_stop_for_kick(info["reached_ball"], linvel),
        "orient_to_ball": self._reward_orient_to_ball(ball_pos_local),
        "ball_proximity": self._reward_ball_proximity(ball_pos_local, info["reached_ball"]),
        "ball_height": self._reward_ball_height(data.site_xpos[self._ball_site_id]),
        ...
        "aligned_approach": self._reward_aligned_approach(data, info, linvel),
        ...
        "feet_phase": self._reward_feet_phase(
            data, info["phase"], self._config.reward_config.max_foot_height,
        ),
  ...
  def _reward_move_to_ball(
      self, ball_pos_local: jax.Array, linvel: jax.Array
  ) -> jax.Array:
    ball_dir = ball_pos_local / (jp.linalg.norm(ball_pos_local) + 1e-6)
    speed_toward = jp.dot(linvel[:2], ball_dir)
    return jp.clip(speed_toward / self._config.too_fast_threshold, 0.0, 1.0)
  ...
  def _reward_ball_speed(
      self, ball_linvel: jax.Array, max_ball_speed: jax.Array, info: dict
  ) -> jax.Array:
    speed = jp.linalg.norm(ball_linvel)
    xy_speed = jp.linalg.norm(ball_linvel[:2])
    cos_angle = jp.dot(ball_linvel[:2], info["kick_dir_world"]) / (xy_speed + 1e-6)
    cos_threshold = jp.cos(jp.array(15.0 * jp.pi / 180.0))
    moving = speed > 0.05
    correct_dir = cos_angle > cos_threshold
    speed_reward = jp.minimum(speed / max_ball_speed, 1.0) ** 2
    reward = jp.where(correct_dir, speed_reward, -0.05)
    return reward * moving

  def _reward_kick_foot_velocity(
      self, feet_vel: jax.Array, reached_ball: jax.Array
  ) -> jax.Array:
    kick_foot_vel = jp.linalg.norm(feet_vel[:3])
    return kick_foot_vel * reached_ball

  def _reward_kick_dir_bonus(
      self, data: mjx.Data, info: dict[str, Any]
  ) -> jax.Array:
    ball_linvel = data.sensordata[self._ball_linvel_sensor_adr]
    speed = jp.linalg.norm(ball_linvel)
    moving = speed > 0.1
    xy_speed = jp.linalg.norm(ball_linvel[:2])
    cos_angle = jp.dot(ball_linvel[:2], info["kick_dir_world"]) / (xy_speed + 1e-6)
    cos_threshold = jp.cos(jp.array(15.0 * jp.pi / 180.0))
    correct_dir = cos_angle > cos_threshold
    return (correct_dir & moving).astype(jp.float32)
  ...
  def _reward_orient_to_kick_dir(
    ...
    cos_align = jp.dot(fwd_xy, info["kick_dir_world"])
    return jp.exp(-jp.square(jp.arccos(jp.clip(cos_align, -1.0, 1.0))) / 0.2)
  ...
  def _reward_sidestep(
    ...
    dist = jp.linalg.norm(ball_pos_local)
    in_circle = (dist > 0.42) & (dist < 0.65)
    torso_mat = data.site_xmat[self._site_id].reshape(3, 3)
    fwd_xy = torso_mat[:2, 0] / (jp.linalg.norm(torso_mat[:2, 0]) + 1e-6)
    not_aligned = jp.dot(fwd_xy, info["kick_dir_world"]) < jp.cos(
        jp.array(10.0 * jp.pi / 180.0)
    )
    cross = fwd_xy[0] * info["kick_dir_world"][1] - fwd_xy[1] * info["kick_dir_world"][0]
    correct_lateral = -jp.sign(cross) * linvel[1]
    lateral_speed = jp.minimum(jp.clip(correct_lateral, 0.0, None) / 0.2, 1.0)
    facing_ball = jp.abs(jp.arctan2(ball_pos_local[1], ball_pos_local[0])) < jp.array(45.0 * jp.pi / 180.0)
    return lateral_speed * in_circle * not_aligned * facing_ball

  def _reward_aligned_approach(
      self,
      data: mjx.Data,
      info: dict[str, Any],
      linvel: jax.Array,
    ...
    torso_mat = data.site_xmat[self._site_id].reshape(3, 3)
    fwd_xy = torso_mat[:2, 0] / (jp.linalg.norm(torso_mat[:2, 0]) + 1e-6)
    cos_threshold = jp.cos(jp.array(45.0 * jp.pi / 180.0))
    facing_kick = jp.dot(fwd_xy, info["kick_dir_world"]) > cos_threshold
    facing_ball = jp.abs(jp.arctan2(ball_pos_local[1], ball_pos_local[0])) < jp.array(45.0 * jp.pi / 180.0)
    forward_speed = jp.clip(linvel[0] / self._config.too_fast_threshold, 0.0, 1.0)
    return forward_speed * (facing_kick & facing_ball).astype(jp.float32)
  ...
  def _cost_wrong_approach(
    ...
    fwd_xy = torso_mat[:2, 0] / (jp.linalg.norm(torso_mat[:2, 0]) + 1e-6)
    cos_threshold = jp.cos(jp.array(45.0 * jp.pi / 180.0))
    not_facing_kick = jp.dot(fwd_xy, info["kick_dir_world"]) < cos_threshold
    ball_angle = jp.arctan2(ball_pos_local[1], ball_pos_local[0])
    not_facing_ball = jp.abs(ball_angle) > jp.array(45.0 * jp.pi / 180.0)
    in_sweet_spot = (dist > 0.42) & (dist < 0.65)
    return (~in_sweet_spot & (not_facing_kick | not_facing_ball)).astype(jp.float32)
  ...
  def _reached_ball(self, ball_pos_local: jax.Array) -> jax.Array:
    ...
    return jp.logical_and(dist < 0.42, jp.abs(angle) < jp.pi / 4)
